...
const Editor = ({
  ...
  const quillReff = useRef<Quill | null>(null);
  ...
  useEffect(() => {
    ...
    quillReff.current = quill;
    quillReff.current.focus();
    ...
    <div className="flex flex-col">
          ...
          {variant === "update" && (
            <div className="ml-auto flex items-center gap-x-2">
              ...
              <Button
                ...
                onClick={() => {
                  onSubmit({
                    body: JSON.stringify(quillReff.current?.getContents()),
                    image
                  })
                }}
                size="sm"
      ...
      {variant === "create" && (
        <div className={cn("p-2 text-[10px] text-muted-foreground flex justify-end opacity-0 transition", !isEmpty && "opacity-100")}>
          <p><strong>Shift + Enter </strong> to add new line</p>
        </div>
      )}
    </div>
  )
